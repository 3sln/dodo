...
- **Hot Path Allocation:** Reconciling one element walks its props, styling, attrs, dataset and hooks. Building an array of `[name, value]` pairs for each of those maps, on every update, is the largest single source of garbage in a render — measured at ~212 KB per update against ~33 KB for the visitor path, over 200 rows of five props. So the reconciler iterates through `mapEach(map, visit, a, b, c)`, which never materialises entries. Visitors are hoisted to the factory closure and take their context through the `a`/`b`/`c` slots, so iterating allocates neither entries nor a closure. Keep new visitors hoisted.
