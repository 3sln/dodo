...
- **Factory Pattern:** Module factories take a `userSettings` object carrying a `dodo` instance — `reactive({dodo})`, `context({dodo, reactive})` — matching the shape used across the project. `src/settings.js` resolves defaults. Factories are not memoised and must not become so: dependencies between modules are injected explicitly instead. `context` needs a `watch`, so it takes one, and returns whichever it ended up using. This matters because a `special` component's identity is its descriptor object and the reconciler uses that identity for node reuse — two independently built `watch` components never reuse each other's nodes. The bundled `context.js` entry point imports `watch` from `reactive.js` and injects it, which is why both entry points expose the same component.
- **Settings:** `dodo` (required), `schedule`, `renderError`, and `reactive` for `context`. Map handling and change detection are not overridable — they always come from `dodo.settings`, because a module must agree with its renderer about what a map is.
